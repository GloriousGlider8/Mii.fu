...
class ZlibImpl(abc.ABC):
	"""ZLib compression implementation.

	Must be overrided using a library for your target language!"""

	def decompress(self, input: bytearray | bytes, output: bytearray, window_bits: int, compressed_size: int, original_size: int) -> None:
		"""Write the decompressed contents of `input` into `output`.

		`windowBits` and `originalSize` must also be specified."""

	def compress(self, input: bytearray | bytes, output: bytearray, level: int, window_bits: int) -> None:
...
class FFLiResourcePartsInfo:
	...
	def load_header(self, data: ByteReader, data_buffer: bytearray, compressed_buffer: bytearray, compression_implementation: ZlibImpl) -> None:
		...
		compression_implementation.decompress(compressed_buffer, data_buffer, WindowBitConverter.f_f_li_resource_window_bits_to_zlib_window_bits(self.window_bits), self.compressed_size, self.uncompressed_size)
		self._b_reader = self.part_data.load_header(data_buffer)
		data.seek(o_offset)
